...
@interface RoomViewController () <WDGVideoDelegate, WDGConversationDelegate, WDGConversationStatsDelegate>
...
- (IBAction)actionButtonTapped:(id)sender {
    if (self.remoteStream == nil) {
        ...
        [self.actionButton setTitle:@"用户列表" forState:UIControlStateNormal];
        [self.videoConversation close];
        [self clearStatsLabels];
        [self.remoteStream detach:self.remoteVideoView];
        self.remoteStream = nil;
...
- (void)wilddogVideo:(WDGVideo *)video didReceiveCallWithConversation:(WDGConversation *)conversation data:(NSString *)data {
    ...
    UIAlertAction *acceptAction = [UIAlertAction actionWithTitle:@"接受" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [strongSelf.actionButton setTitle:@"挂断" forState:UIControlStateNormal];
    }];
...
- (void)setVideoViewDisplayMode {
    ...
    self.remoteVideoView.mirror = YES;
...
- (void)createLocalStream {
    ...
    self.localStream = [self.wilddogVideoClient localStreamWithOptions:localStreamOptions];
}
